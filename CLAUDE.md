...
- [ ] **학기 시작 전 용량 점검** (2026-08-12 제기, 미완) — 실사용 50명이면 하루 150장. 지금 못 쟀다(파일이 0개라 `storage.objects`가 비어 있었음 — 정상). 사진 몇 장 올린 뒤:
  ```sql
  select bucket_id, count(*) files,
         pg_size_pretty(sum((metadata->>'size')::bigint)) total,
         pg_size_pretty(avg((metadata->>'size')::bigint)::bigint) avg_file
  from storage.objects group by bucket_id;
  ```
  - **`50MB`는 파일당 제한**이라 우리 사진은 안 걸린다. 진짜 위험은 ① **아카이브 무한 누적**(지우는 코드가 없다) ② **대역폭** — 핀 하나 누를 때마다 원본을 통째로 전송하는데 팝업은 200px로 줄여 보여준다. 한도는 대시보드 Settings → Usage에서 확인
  - **필름 그레인이 파일 크기를 키웠다.** JPEG은 비슷한 색이 뭉친 곳을 잘 줄이는데 그레인은 정확히 그 반대다. 예상보다 크면 이게 범인
  - 조치 후보: `reset_daily()`에 "N일 지난 아카이브 삭제" 추가 / 썸네일 저장 또는 Supabase 이미지 변환
- [ ] 데모 스크린샷 → 레쥬메 업데이트
