...
- 각인은 **좌표가 도착한 뒤 사진 위에 덧그린다** (2026-08-18에 흰 여백 → 사진 안쪽 아래 가운데로 이동). GPS는 셔터 몇 초 뒤에 오는데, 캔버스는 이미 그린 픽셀 위에 덧칠하는 방식이라 사진을 다시 그릴 필요가 없다. 흰 여백은 실물 폴라로이드처럼 비워둬야 "종이"로 읽힌다
- **좌표 대신 지명 두 줄**: `● COLLEGE AVE, NEW BRUNSWICK` / `AUG. 18 - 9:04 PM` (레퍼런스 `docs/ref1.heic`). 흰 글자 + 그림자(밝은 사진에서 글자가 사라진다), 초록 점은 "지금 여기" 신호. 점+글자를 한 덩어리로 재서 가운데 정렬해야 함 — `textAlign="center"`만 쓰면 글자만 가운데라 점 때문에 왼쪽으로 밀려 보인다
- **지오코딩 API를 안 쓴다**: 우리 Mapbox 토큰은 타일만 허용돼 있어 geocoding이 `Forbidden`. 게다가 러트거스 전용 앱이라 나올 답이 정해져 있어서 `AREAS` 표(6곳, 반경 800m 최근접)로 충분하다. 캠퍼스 밖이면 지명 줄 없이 시각만
