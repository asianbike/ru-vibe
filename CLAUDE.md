...
- [x] 13. **히트맵** (2026-08-04) — `/map`에 열지도 layer. **마커를 안 갈아엎은 이유**: 열지도는 지도 캔버스 *안*에 그려지고 핀은 그 위에 얹힌 HTML이라 층이 원래 다르다. 덕분에 태스크 7·8 코드가 한 줄도 안 바뀌었다
  - `addSource`/`addLayer`는 **`map.on("load")` 안에서만** — 스타일은 네트워크로 받아오는 것이라 지도를 만들자마자 부르면 터진다. 조회가 먼저 끝날 수도 있어서 `syncHeat()`를 load에서 한 번 더 부른다
  - `heatmap-color`의 밀도 0은 **반드시 완전 투명**. 안 그러면 게시물 없는 캠퍼스 전체가 옅게 물든다
  - `heatmap-opacity`가 zoom 15.5에서 0 — 가까이서는 "어디가 뜨거운가"가 아니라 "이 사진 뭐야"가 관심사
  - 다이얼: `heatmap-intensity`/`-radius`(둘 다 zoom별 `interpolate`). 게시물이 적을 땐 intensity를 올려야 보인다
  - `@types/geojson`을 안 깔고 `HeatFeature` 6줄을 직접 적음
- [x] 14. **공유** (2026-08-04) — 게시 후 `navigator.share({ files: [폴라로이드] })`. **인스타를 직접 여는 게 아니라 OS 공유 시트에 맡긴다** — `instagram-stories://` 스킴은 앱 설치 여부를 알 수 없어 안 깔린 유저에겐 아무 일도 안 일어난다. `navigator.canShare({files})`로 지원 여부를 확인해 안 되는 브라우저(데스크톱 Chrome 등)에선 버튼을 아예 안 그린다
