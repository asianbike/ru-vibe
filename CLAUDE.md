...
- 각인은 **좌표가 도착한 뒤 아래 흰 여백에 덧그린다.** GPS는 셔터 몇 초 뒤에 오는데 여백은 빈 종이라 덧그리기만 하면 되고 사진을 다시 그릴 필요가 없다. (2026-08-18에 사진 위 흰 글자로 옮겨봤다가 하루 만에 되돌림 — 밝은 사진에서 그림자를 넣어도 글자가 사진을 가린다)
- **좌표 대신 지명 두 줄**: `● COLLEGE AVE, NEW BRUNSWICK` / `AUG. 18 - 9:04 PM` (레퍼런스 `docs/ref1.heic`). Helvetica Neue **Bold**, 검정(`#111`), 크기는 사진 한 변의 5.2%. 초록 점은 "지금 여기" 신호. 점+글자를 한 덩어리로 재서 가운데 정렬해야 함 — `textAlign="center"`만 쓰면 글자만 가운데라 점 때문에 왼쪽으로 밀려 보인다. 지명이 없으면 시각 한 줄을 여백 한가운데에
- **지오코딩 API를 안 쓴다**: 우리 Mapbox 토큰은 타일만 허용돼 있어 geocoding이 `Forbidden`. 게다가 러트거스 전용 앱이라 나올 답이 정해져 있어서 `AREAS` 표(6곳 최근접)로 충분하다. **반경은 2km**(`AREA_RADIUS`) — 800m로 시작했더니 캠퍼스에서 1km 떨어진 집에서 지명 줄이 통째로 빠졌다. 2km는 "가장 가까운 캠퍼스"라는 뜻이 되지만 지도 핀은 진짜 좌표를 쓰므로 어긋남은 각인 문구에만 있다
- **사진에 좌표를 안 새긴다**(2026-08-18 이전엔 3자리). 화면에는 여전히 5자리. 이 이미지는 public URL로 나가고 인스타로도 퍼진다 — 지명은 동네 수준이라 집이 안 박힌다
