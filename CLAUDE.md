...
- [x] 15. **첫 방문 안내 카드** (`components/Welcome.tsx`) — 링크 하나로 들어오는 웹앱이라 "이게 뭔지" 알려주는 단계가 없었다. 한 번 닫으면 localStorage에 기록되고 다시 안 뜬다(설치 안내와 달리 접어두지 않는다 — "이 앱이 뭔가"는 한 번 알면 끝)
- [x] 16. **어드민** (`20260812000000_admins.sql`) — 하루 3장 면제. 히트맵 시드용. **uid는 레포에 안 박고** 대시보드에서 넣는다. `admins`에 RLS를 켜면 select 정책이 **필수** — 없으면 `can_post()`가 그 표를 못 읽어서 관리자도 조용히 3장에서 막힌다(함수는 부른 사람 권한으로 돈다)
- [ ] **폰 실기기 QA**: 촬영→Post→핀, 홈 화면 설치, 인앱 배너, 안내 카드, 새 폴라로이드 룩
- [ ] 06:00 리셋이 **새 키 + `apikey` 헤더**로 실제로 도는지 `net._http_response`로 확인 (그 조합의 첫 실전)
- [ ] **사진 용량 — 미해결** (2026-08-12). 실제 규모는 50명이 아니라 **10명 남짓**으로 하향. 그래도 아래 둘은 크기와 무관하게 언젠가 터진다:
  - ① **아카이브가 영원히 쌓인다** — `reset_daily()`에 지우는 코드가 없다. 규모와 상관없이 필요한 조치
  - ② 지도 팝업이 **원본을 통째로 받아 200px로 줄여 보여준다** — 썸네일이 있으면 전송량이 한 자릿수 배로 준다
  - **측정 방법**:
    ```sql
    select name, (metadata->>'size')::int size, created_at
    from storage.objects where bucket_id='photos' order by created_at desc limit 5;
    ```
    (`created_at`은 UTC — 방금 올린 게 맞는지 환산해서 볼 것)
  - **막힌 지점**: 700KB → `PHOTO_PX` 1100→900, JPEG 품질 0.85→0.75 로 낮췄는데 다음 측정이 **623KB**. 기대값은 300KB 근처였다. 후보 둘 — (a) 잰 파일이 변경 **전에** 올라간 것 (b) 정말 안 줄어든 것. **`created_at`으로 갈리는데 아직 안 봤다.** 여기서부터 재개
  - **`50MB`는 파일당 제한**이라 우리 사진은 안 걸린다. 헷갈렸던 부분
  - **필름 그레인이 파일을 키운 원인.** JPEG은 비슷한 색이 뭉친 곳을 잘 줄이는데 그레인은 픽셀마다 값이 튀어서 정확히 그 반대다
- [ ] 데모 스크린샷 → 레쥬메 업데이트
- [ ] 12. **이해 점검 문답** — 맨 마지막. **선행 자료 `docs/basics.md` 작성 완료(2026-08-04, 커밋 안 됨)** — 서버/클라이언트·요청 헤더·React state/effect·버킷·RLS·키·비동기·PWA를 완전 기초부터 쓴 것. 작업자가 *"버킷이 뭔지도 모르겠다"* 고 해서 만듦. 문답은 이 문서를 읽은 다음 진행하고, 맨 뒤 12개 질문을 출발점으로 쓸 것. 본인 표현: *"이 프로젝트를 완벽하게 이해하지 못하고 있는 것 같다"*. 목표: **면접에서 "이 프로젝트 설명해보세요"에 막힘없이 답하기**. 주제:
  0. **RLS 회귀 테스트가 잡는 것은 유저가 아니라 나다** (2026-08-12 오해 정정) — 유저는 정책을 못 고친다. 잡는 건 "새 기능을 만들다 옛 보장을 깨뜨리는 것". 화면은 멀쩡히 돌아가서 눈에 안 보인다
...
**다음 프로젝트용 `CLAUDE.md` 초안이 `docs/next-project.md`에 있다** — 이번 방식(랩·퀴즈·좁히기)을 프로젝트 무관하게 뽑아둔 것. 새 레포에 복사해서 대괄호만 채우면 시작.
