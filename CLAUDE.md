...
- **촬영 가능 시간**: 제한 없음, 24/7 오픈 (2026-07-28 결정 — 낮 시간 캠퍼스 이벤트도 놓치지 않기 위해)
- **하루 3장 제한**: INSERT 시점에 RLS 정책/Postgres 함수로 카운트 체크 (America/New_York 06:00 리셋 기준)
...
- **사진 aesthetic 고도화** — 태스크 8(지도에 핀+사진 표시) 이후에. 룩 결정 코드는 `PolaroidCanvas`의 `drawImage` 이후 10줄에 전부 모여 있고, 앱의 다른 곳은 사진 생김새에 의존하지 않아서 언제든 갈아끼울 수 있음. 사진은 매일 06:00 삭제되므로 "옛날 필터로 남은 사진" 문제도 없음. 후보: 진짜 폴라로이드 프레임(아래 흰 여백+정사각형), 비네팅, 필름 그레인, 색보정(`ctx.filter`), 커스텀 폰트. `MAX_EDGE=2048`은 이 작업에 필요한 해상도를 남겨둔 값이니 더 줄이지 말 것.
